...
for ele in ll:
	allTogether+=ele		
if ("PF00013" in allTogether) and ("PF13014" in allTogether) and("PF12235" in allTogether):
	wtf=1

for element in ll:
	print element
	if (wtf==1)and(int(element.split()[1])-int(element.split()[2])<=54) and (int(element.split()[1])-int(element.split()[2])>0):#### for FMRP iso7  meinetwegen bitte weg damit
