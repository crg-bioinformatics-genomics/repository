...
with open(os.path.join(SCRIPT_PATH, "catrapid_graphic.yaml"), "r") as task_f:
	task_definition = yaml.load(task_f)

parser = argparse.ArgumentParser(
   description='Launches catRAPID graphic with properly parset parameters')
...
protLen = len(str(protSeq[0].seq))

	HTML_PATH="index.html"
	cmd = """bash RBProfiles.sh "{}" "{}" "{}" """.format(random_number, title, protFile)
